#!/usr/bin/env python3
"""
Caching information of function call history to redis
"""
...
def count_calls(method: Callable) -> Callable:
    """
    decorator that counts how many time a function has been called
    """
    @wraps(method)
...
def call_history(method: Callable) -> Callable:
    """
    decorator to store methods input and output history
    """
    @wraps(method)
...
def replay(function: callable) -> None:
    """
    replay's the call history of a function
    """
...
class Cache:
    ...
    def store(self, data: Union[int, float, bytes, str]) -> str:
        """
        store data in redis using a random key and data as value
        """
        key = str(uuid4())
